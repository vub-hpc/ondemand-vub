...
# Origin: https://github.com/OSC/bc_osc_tensorboard
#
...
class TokenResource(Resource):
    ...
    def getChild(self, path, request):
        ...
        try:
            ood_token = os.environ.get(f"_ood_token_{ood_session_id.replace('-', '_')}")
        except:
            ood_token = None

        # check that cookie matches the local token
        if cookie == ood_token and cookie is not None:
            return proxy.ReverseProxyResource(self.host, self.port, b"/" + path)
...
def parse_args(argv=sys.argv[1:]):
    ...
    parser.add_argument(
        "--app-port", dest="app_port", type=int, required=True, help="application port, to forward requests to"
    )
    parser.add_argument(
        "--proxy-port", dest="proxy_port", type=int, required=True, help="proxy port, to listen for requests on"
    )
    parser.add_argument(
        "--bind-host", dest="bind_host", type=str, required=False, help="the host to bind on", default="127.0.0.1"
    )
...
def main(argv=sys.argv[1:]):
    ...
    print(f"authrevproxy is starting on port {args.proxy_port} -> {args.app_port} and bind on {args.bind_host}")
